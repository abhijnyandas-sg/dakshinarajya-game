...
exports.DEFAULT_GOLD_GOAL = exports.DEFAULT_BANNER_GOAL = exports.MAX_PLAYERS = exports.PASS_COURT_GOLD = exports.START_GOLD = void 0;
exports.raniBonus = raniBonus;
exports.createGame = createGame;
exports.newPlayerId = newPlayerId;
exports.addPlayer = addPlayer;
exports.findPlayer = findPlayer;
exports.currentPlayer = currentPlayer;
exports.chooseRani = chooseRani;
exports.startGame = startGame;
exports.roll = roll;
exports.buyCity = buyCity;
exports.frontier = frontier;
exports.useDecree = useDecree;
exports.endTurn = endTurn;
exports.villageCount = villageCount;
...
exports.START_GOLD = 1500;
exports.PASS_COURT_GOLD = 200;
exports.MAX_PLAYERS = 6;
exports.DEFAULT_BANNER_GOAL = 10; // villages for a Mandala victory
exports.DEFAULT_GOLD_GOAL = 6000; // economic victory
let _seq = 0;
function uid(prefix) {
    _seq += 1;
...
function raniBonus(raniId) {
    const r = raniId ? ranis_1.RANI_BY_ID[raniId] : null;
    if (!r)
        return { ...ZERO };
    const b = { ...ZERO };
    switch (r.archetype) {
...
function createGame(code, hostId, hostName) {
    const s = {
        ...
        log: [],
        winnerId: null,
        bannerGoal: exports.DEFAULT_BANNER_GOAL,
        goldGoal: exports.DEFAULT_GOLD_GOAL,
        round: 1,
    };
    for (const id of Object.keys(palegallu_1.PALEGADU_BY_ID))
        s.villages[id] = null;
...
function addPlayer(s, id, name, isHost = false, isBot = false) {
...
function chooseRani(s, playerId, raniId) {
    if (s.phase !== "lobby")
        return;
    if (!ranis_1.RANI_BY_ID[raniId])
        return;
    ...
    if (!p)
        return;
    p.raniId = raniId;
    log(s, `${p.name} pledges to ${ranis_1.RANI_BY_ID[raniId].name}.`);
}
function startGame(s, requesterId) {
    if (s.phase !== "lobby")
        return;
    if (requesterId !== s.hostId)
        return;
    ...
    for (const p of s.players) {
        if (!p.raniId) {
            const free = Object.keys(ranis_1.RANI_BY_ID).find((id) => !s.players.some((q) => q.raniId === id));
...
function tollFor(space) {
    return Math.round((space.price ?? 0) * 0.1);
}
function resolveLanding(s, p, passedCourt) {
    const bonus = raniBonus(p.raniId);
    if (passedCourt) {
        const g = exports.PASS_COURT_GOLD + bonus.circuitGold;
        p.gold += g;
        p.grain += bonus.passCourtGrain;
        log(s, `${p.name} passes the Royal Court (+${g} Gold).`);
    }
    const sp = board_1.SPACES[p.pos];
    switch (sp.t) {
        case "tax": {
            if (p.taxImmune) {
                p.taxImmune = false;
        ...
        case "festival": {
            ...
            log(s, `${sp.name}: ${p.name} gains ${g} Gold, +1 Grain${bonus.festivalScholar ? ", +1 Scholar" : ""}.`);
            break;
        }
        case "yagna": {
            p.merit += 1;
        ...
        case "route": {
            ...
            if (!owner) {
                ...
                const sameGroup = owner.owned.filter((oid) => board_1.SPACES[oid].g && board_1.SPACES[oid].g === sp.g).length;
        ...
        case "corner": {
            ...
            if (sp.sub === "banish")
                ...
                log(s, `${p.name} halts at ${sp.name}.`);
            break;
        }
    }
}
function roll(s, playerId) {
    if (s.phase !== "playing")
        return false;
    ...
    const steps = a + b;
    const before = p.pos;
    const after = (before + steps) % board_1.BOARD_SIZE;
    const passedCourt = before + steps >= board_1.BOARD_SIZE && after !== 0;
    p.pos = after;
...
function buyCity(s, playerId) {
    if (s.phase !== "playing")
        return false;
    const p = currentPlayer(s);
    if (!p || p.id !== playerId || s.step !== "action")
        return false;
    const sp = board_1.SPACES[p.pos];
    if (sp.t !== "prop" && sp.t !== "route")
        return false;
    if (s.players.some((q) => q.owned.includes(sp.id)))
        return false;
...
function applyPerTurnIncome(s, p) {
    const parts = [];
...
// ---- frontier (Palegallu) ------------------------------------------
function negotiationCost(disposition) {
    if (disposition === "Tribute-friendly")
        return 50;
    if (disposition === "Proud")
        return 100;
    return 150; // Hostile
}
function negotiationThreshold(disposition) {
    if (disposition === "Tribute-friendly")
        return 3;
    if (disposition === "Proud")
        return 6;
...
function applyReward(s, p, effects) {
    const parts = [];
...
function frontier(s, playerId, mode, villageId, astrasSpent = 0) {
    if (s.phase !== "playing")
        return false;
    const p = currentPlayer(s);
    if (!p || p.id !== playerId)
        return false;
    if (s.step !== "action" || s.frontierUsed)
        return false;
    const v = palegallu_1.PALEGADU_BY_ID[villageId];
    if (!v)
        return false;
    if (s.villages[villageId] !== null)
    ...
    if (mode === "negotiate") {
        ...
        log(s, `${p.name} treats with ${v.name} (${v.disposition}) — ${rollv} vs ${need}: ${won ? "pledged!" : "refused."}`);
    }
    else {
        ...
        won = rollv > need;
        if (!won && bonus.keepTroopsOnFail)
            p.astras += spend; // troops kept
        log(s, `${p.name} storms ${v.name} (garrison ${v.garrison}) — ${rollv} vs ${need}: ${won ? "conquered!" : "repulsed."}`);
    }
    if (won) {
...
// ---- decrees --------------------------------------------------------
function openVillages(s) {
    return Object.keys(s.villages).filter((id) => s.villages[id] === null);
}
function pledgeOne(s, p, prefer) {
    const open = openVillages(s);
    const pick = open.find((id) => palegallu_1.PALEGADU_BY_ID[id].disposition === (prefer ?? "Tribute-friendly")) ??
        open[0];
    if (!pick)
        return;
    s.villages[pick] = p.id;
    p.villagesWon.push(pick);
    applyReward(s, p, palegallu_1.PALEGADU_BY_ID[pick].reward);
    log(s, `${p.name} pledges ${palegallu_1.PALEGADU_BY_ID[pick].name} by royal decree.`);
}
function useDecree(s, playerId) {
    if (s.phase !== "playing")
        return false;
    const p = currentPlayer(s);
    if (!p || p.id !== playerId || p.decreeUsed)
        return false;
    const r = p.raniId ? ranis_1.RANI_BY_ID[p.raniId] : null;
    if (!r)
        return false;
    p.decreeUsed = true;
...
function endTurn(s, playerId) {
    ...
    if (!p || p.id !== playerId)
        return false;
    if (!s.rolledThisTurn)
        return false; // must roll before ending
    if (p.shieldTurns > 0)
        p.shieldTurns -= 1;
...
function setDisconnected(s, playerId) {
    const p = findPlayer(s, playerId);
    if (p)
        p.connected = false;
}
